...
namespace Segmentation
{
	public class Manager : MonoBehaviour
	{
		...
		{
			var segmentationTag = target?.GetComponentInChildren<Tag>();
			if (segmentationTag == null)
			{
				segmentationTag = target?.gameObject.AddComponent<Tag>();
				yield return null;
			}
		...
		public void AddClass(in string className, in Tag tag)
		{
			...
			{
				_labelInfo.TryAdd(className, new List<Tag> { tag });
			}
			// Debug.Log($"AddClass: {className}, {_labelInfo[className]}");
		}

		public void RemoveClass(in string className, in Tag tag)
		{
			if (_labelInfo.ContainsKey(className))
			{
		...
		public Dictionary<string, List<Tag>> GetLabelInfo()
		{
