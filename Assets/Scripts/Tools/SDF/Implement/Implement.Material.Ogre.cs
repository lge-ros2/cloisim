...
namespace SDF
{
	namespace Implement
	{
		...
		{
			private static string FindFilePath(in List<string> uris, in string targetFileName)
			{
				...
				in List<string> uris)
			{
				foreach (var textureUnitEntry in textrueUnits)
				{
					...
					var textureFilePath = FindFilePath(uris, textureFileName);
					...
					material.ApplyTexture(textureFilePath, textureUnitProps);
					return;
				}
			}

			private static UE.Material[] ResizeMaterials(this OgreMaterial.Material ogreMaterial, in UE.Material[] baseMaterials)
			{
			...
			public static UE.Material[] ApplyMaterial(
				this OgreMaterial.Material ogreMaterial,
				in UE.Material[] baseMaterials,
				in List<string> texturePathURIs)
			{
				var materials = ogreMaterial.ResizeMaterials(baseMaterials);
