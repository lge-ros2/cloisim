...
{
	// Precompiled protobuf-net serializer (see Assets/Editor/ProtoMessagesCompiler.cs).
	...
	private static readonly CloisimProtoMessages _protoMessages = new();
	...
	public bool SetMessage<T>(T instance)
	{
		...
		try
		{
			_protoMessages.Serialize(this, instance);
			return true;
		}
		catch (Exception ex)
		{
	...
	public bool TryGetMessage<T>(out T result)
	{
		...
		try
		{
			result = _protoMessages.Deserialize<T>(this, default);
			return true;
		}
		catch (Exception)
		{
