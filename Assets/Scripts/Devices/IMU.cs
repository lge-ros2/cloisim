...
namespace SensorDevices
{
	public class IMU : Device
	{
		...
		public void SetupNoises(in SDF.IMU element)
		{
			...
			{
				_noises.angular_velocity["x"] = new SensorDevices.Noise(element.noise_angular_velocity.x);
			}

			if (element.noise_angular_velocity.y != null)
			{
				_noises.angular_velocity["y"] = new SensorDevices.Noise(element.noise_angular_velocity.y);
			}

			if (element.noise_angular_velocity.z != null)
			{
				_noises.angular_velocity["z"] = new SensorDevices.Noise(element.noise_angular_velocity.z);
			}

			if (element.noise_linear_acceleration.x != null)
			{
				_noises.linear_acceleration["x"] = new SensorDevices.Noise(element.noise_linear_acceleration.x);
			}

			if (element.noise_linear_acceleration.y != null)
			{
				_noises.linear_acceleration["y"] = new SensorDevices.Noise(element.noise_linear_acceleration.y);
			}

			if (element.noise_linear_acceleration.z != null)
			{
				_noises.linear_acceleration["z"] = new SensorDevices.Noise(element.noise_linear_acceleration.z);
			}
		}
