...
/// <summary>
/// Precompiles a static protobuf-net serializer assembly (CloisimProtoMessages.dll)
...
public static class ProtoMessagesCompiler
{
	...
	public static void Compile()
	{
		...
		{
			Debug.LogError("ProtoMessagesCompiler: no cloisim.msgs [ProtoContract] types found — aborting.");
			return;
		}
		...
		try
		{
			...
			Debug.Log($"ProtoMessagesCompiler: compiled {messageTypes.Count} message types into {OutputPath}");
			AssetDatabase.Refresh();
		}
		catch (Exception ex)
		{
			Debug.LogError($"ProtoMessagesCompiler: Compile() failed: {ex.GetType().FullName}: {ex.Message}\n{ex.StackTrace}");
		}
		finally
		{
