...
if [ -n "$targetWorld" ] && ! WorldValidationCheck "$targetWorld" ; then
  echo -e "\n Invalid world file name or World file NOT exist (falling back to world picker).\n"
  PrintWorldList
  targetWorld=""
fi

if [ -z "$targetWorld" ] && $enableHeadless ; then
  echo -e "\n Headless mode requires a valid -world file (no UI world picker available).\n"
  PrintWorldList
  exit 1
fi

if [ -z "$targetWorld" ]; then
  echo -e "\n No world specified - launching CLOiSim world picker...\n"
fi
...
worldArgs=()
if [ -n "$targetWorld" ]; then
  worldArgs=("-world" "$targetWorld")
fi
...
# VK_LOADER_DEBUG=all \
./CLOiSim.x86_64 "${headlessArgs[@]}" "${worldArgs[@]}" "${captureArgs[@]}"
CLOISIM_EXIT_CODE=$?
