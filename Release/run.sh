    ...
    if WorldValidationCheck $targetWorld ; then
      ...
      CollectCrashDump()
      {
        ...
        if [[ -n "${LOG_FILE}" && -f "${LOG_FILE}" ]]; then
          cp "${LOG_FILE}" "${DUMP_DIR}/CLOiSim_local.log"
        fi
      ...
      LOG_FILE="./CLOiSim_$(date '+%Y%m%d_%H%M%S').log"
      ./CLOiSim.x86_64 -logFile "${LOG_FILE}" "${headlessArgs[@]}" -world "$targetWorld" "${captureArgs[@]}"
      CLOISIM_EXIT_CODE=$?
